...
<div class="main_right">
     ...
     <div class="person_page">
          <ul>
              ...
              <li class="onpage" id="fifthPage">第五页</li>
          </ul>
    </div>
     ...
     <input style="display:none" id="type" value ="certificate"/>
     <div class="main_con marb_90">
         ...
         <div class="personal_info clearfix">
              <div class="person_title font24">毕业证书</div>
              ...
              <table width="100%" class="student_archives">
                 ...
                 <td>
                     <div class="show_file">
                          <div class="clearfix IdentifyEdit"><p class="fl">添加附件：</p><p class="fl"><input type="file" name="file" id="file" class="search_box boxbor"></p></div>
                          <c:if test="${empty(requestScope.attachList)}">
                           	<p class="upload_certificate"><img src="${basePath}resources/images/nopic.jpg"></p>
                          </c:if> 
                          <c:forEach items="${attachList}" var="item" varStatus="status">
                          <p class="upload_certificate"><img src="${imgBasePath}${item.urlAddress}"></p>
                          <p><a href="javascript:void(0);" class="color_red deleteAttach"  data-attachid="${item.id}">删除</a></p>
                          </c:forEach>
                          <%-- <c:forEach items="${attachList}" var="item" varStatus="status">
                          <div class="clearfix">
                          <p class="fl">附件：</p><p class="fl"><a href="${imgBasePath}${item.urlAddress}">${item.name}</a></p><p class="fl"><a href="javascript:void(0);" class="color_red deleteAttach"  data-attachid="${item.id}">删除</a></p><p class="fl">上传时间：<fmt:formatDate value="${item.createTime}" type="both" pattern="yyyy-MM-dd HH:mm"/></p></div>
                          </c:forEach> --%>
                     </div>
                 </td>
                 </tr>
