...
(function($){
		...
		dialogHtml : function(opts){
			...
			var dialogHtml = "<div class='b_l w460'><div class='bcom_ti'>"+
			                 ...
			                 "<div class='bcom_cent'>"+
	                 	     ...
	                 	     "<p class='bcoma'><a href='javascript:void(0)' id='bc_abut1' class='bc_abut1 tmui-modal-sure'>"+opts.sureText+"</a>"+
	...
	$.tmLoading = function(content,options){
		...
		var spanId = "tm-loading-content";
		if(opts.type == "error") {
			spanId = "tm-error-content";
		} else if(opts.type == "success") {
			spanId = "tm-success-content"
		} else if(opts.type == "prompt") {
			spanId = "tm-prompt-content";
		}
		if($(".tmui-loading").length==0)$("body").append('<div class="tmui-loading" title="click close me!"><span id="' + spanId + '"></span></div>');
		var $loading = 	$(".tmui-loading");
		if(opts.skin=="black"){
			$(".tmui-loading").css({background:"#3e4146","color":"#fff","border":"1px solid #555"});
		} else if(opts.skin=="white"){
			$(".tmui-loading").css({background:"#eeeeee","color":"#515151","border":"0"});
		}
		if(opts.overlay){
		...
		if(!opts.showLoad)$loading.find("#" + spanId).css("background","none");
		if(content=="remove"){
			$loading.slideUp(250,function(){
			...
			$loading.show().find("#" + spanId).html(content);
