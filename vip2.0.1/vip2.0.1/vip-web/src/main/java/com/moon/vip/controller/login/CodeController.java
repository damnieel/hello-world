...
public class CodeController extends HttpServlet {
	...
	private static final long serialVersionUID = -1043955097180586415L;
	 ...
	 * void
	 */
	@RequestMapping("/codeShow")
	 public void codeShow(HttpServletRequest request, HttpServletResponse response)
	  ...
	  for(int i=0;i<4;i++){
	   ...
	   trans.rotate(random.nextInt(4)*3.14/180,15*i+2,7);
	   //缩放文字
	   float scaleSize=random.nextFloat()+0.6f;
	   if(scaleSize>0.8f){
		   scaleSize=0.8f;
	   }
	   ...
	   /************************/
	   g.drawString(String.valueOf(ctmp),30*i+10,33);

	  }
	  //将生成的验证码保存到Session中
	  HttpSession session=request.getSession();
	  session.setAttribute("codeShow",sRand);
	  g.dispose();
